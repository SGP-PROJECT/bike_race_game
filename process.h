...
void load_layer(int x=0 ,int y=0)
{
     ...
     apply_surface(bg[0].get_cord_x(),0,back[0],screen);
     apply_surface(bg[1].get_cord_x(),0,back[1],screen);
     ...
     apply_surface(bike_x,bike_y,bike,screen);
...
bool bike_process::process()
{
     ...
     if(keystate[SDLK_SPACE] or keystate[SDLK_UP])
     {
          if(jump_state==0)
          jump_state=36;

     }
     if( (bike_y <(bike_stand-bike->h)) or jump_state>=16  )
     {
          int jump_height;
          jump_height=19-jump_state;
          bike_y += jump_height;
          jump_state--;
          if(jump_state==0)
          jump_state--;
